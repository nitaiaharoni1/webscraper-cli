...
class BrowserManager:
    ...
    def _launch_persistent_browser(self, headless: bool = False, proxy: Optional[str] = None) -> int:
        ...
        if self._persistent_process and self._persistent_process.poll() is None and self._persistent_port is not None:
    ...
    async def connect(
        ...
        if mode == "persistent":
            ...
            port = self._launch_persistent_browser(headless=headless, proxy=proxy)
            ...
            context = await pw.chromium.launch_persistent_context(user_data_dir, **launch_options)
...
def get_browser_manager() -> BrowserManager:
    ...
    if _browser_manager is None:
        ...
        atexit.register(_browser_manager._cleanup_temp)
