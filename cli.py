...
from core.progress import set_verbose, set_quiet
...
[bold]Command Categories:[/bold]
  ...
  Extraction:   extract (text, links, html, images, table, social)
  Interaction:  interact (click, type-text, hover, scroll, select, upload)
  ...
  Screenshots:  screenshot (capture, element, fullpage, visual-diff)
  Recording:    record (start, stop, replay, video-start, video-stop)
  Tabs:         tabs (open, close, switch, list, back, forward, history)
  Visual:       visual (responsive, dark-mode, reduced-motion, print-preview)
  Crawling:     crawl (site, sitemap, rss)
  Batch:        batch (urls, selectors, script, retry)
  Flow:         flow (if, exists, loop)
  Serve:        serve (start, proxy)
  And more...
""",
)
...
def global_options(
    ...
    settings.headless = headless  # Default is headed (visible browser)
    
    # Set progress module settings
    set_verbose(verbose)
    set_quiet(quiet)

# Add command groups with help text
app.add_typer(navigate.app, name='navigate', help='Browser navigation: goto, back, forward, reload')
app.add_typer(interact.app, name='interact', help='User interactions: click, type-text, hover, scroll, select, upload, keyboard')
app.add_typer(extract.app, name='extract', help='Data extraction: text, links, html, images, table, social, table-csv')
app.add_typer(screenshot_module.app, name='screenshot', help='Screenshots: capture, element, fullpage, visual-diff')
app.add_typer(wait.app, name='wait', help='Wait commands: selector, timeout, navigation, idle, animation')
app.add_typer(eval_module.app, name='eval', help='JavaScript evaluation: run')
app.add_typer(page.app, name='page', help='Page operations: info, pdf')
app.add_typer(storage.app, name='storage', help='Storage: cookies, localstorage')
app.add_typer(batch.app, name='batch', help='Batch operations: urls, selectors, script, retry')
app.add_typer(crawl.app, name='crawl', help='Web crawling: site, sitemap, rss')
app.add_typer(frames.app, name='frame', help='Frame operations: switch, main, list-frames')
app.add_typer(dialogs.app, name='dialog', help='Dialog handling: accept, dismiss')
app.add_typer(assert_commands.app, name='assert', help='Assertions: exists, text, count, visible')
app.add_typer(daemon.app, name='daemon', help='Daemon mode: start, stop, status, connect')
app.add_typer(content.app, name='content', help='Content extraction: strip, markdown, meta, schema, xpath, regex, forms')
...
app.add_typer(network.app, name='network', help='Network: intercept, requests, headers, auth, throttle, offline, websocket')
app.add_typer(emulate.app, name='emulate', help='Emulation: device, viewport, geolocation')
app.add_typer(forms.app, name='form', help='Form automation: fill, submit')
app.add_typer(scroll_module.app, name='scroll', help='Advanced scrolling: infinite, paginate')
app.add_typer(shadow.app, name='shadow', help='Shadow DOM: access')
app.add_typer(monitor.app, name='monitor', help='Monitoring: console, performance, highlight, record')
app.add_typer(compare.app, name='compare', help='Comparison: snapshot, diff')
app.add_typer(api.app, name='api', help='Browser API: fetch, har, mock')
app.add_typer(perf.app, name='perf', help='Performance: vitals, lighthouse, memory')
app.add_typer(inspect.app, name='inspect', help='Inspection: styles, bounds, contrast, fonts, sw')
app.add_typer(human.app, name='human', help='Human-like interaction: type, mouse, drag')
app.add_typer(audit.app, name='audit', help='Audits: a11y, seo, security, mixed, links, images')
app.add_typer(record.app, name='record', help='Recording: start, stop, replay, video-start, video-stop, video-context')
app.add_typer(tabs.app, name='tabs', help='Tabs: open, close, switch, list, back, forward, history')
app.add_typer(visual.app, name='visual', help='Visual testing: responsive, dark-mode, reduced-motion, print-preview, viewport, contrast')
app.add_typer(flow.app, name='flow', help='Flow control: if, exists, loop')
app.add_typer(serve.app, name='serve', help='Local server: start, proxy')
...
def goto(
    url: str,
    wait_until: str = typer.Option('domcontentloaded', help='Wait until state: load, domcontentloaded, networkidle, commit'),
