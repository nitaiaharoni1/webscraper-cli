...
def type(
    ...
    async def _human_type():
        ...
        output_json({"message": "Typed text with human-like behavior", "selector": selector, "length": len(text)})

    run_async(_human_type())
...
def mouse(
    ...
    async def _human_mouse():
        ...
        if not box:
            output_json({"error": "Element has no bounding box"})
            return
...
def drag(
    ...
    async def _human_drag():
        ...
        if await source_locator.count() == 0:
            output_json({"error": f"Source element not found: {source}"})
            return

        if await target_locator.count() == 0:
            output_json({"error": f"Target element not found: {target}"})
            return
        ...
        output_json({"message": "Dragged element with human-like movement", "source": source, "target": target})

    run_async(_human_drag())
