...
def urls(
    ...
    async def _batch_urls():
        ...
        async def process_url(url: str):
            ...
            connection = await get_or_create_connection(f'{session_id or "batch"}-{hash(url)}', headless=headless_mode)
...
def selectors(
    ...
    async def _batch_selectors():
        ...
        connection = await get_or_create_connection(session_id, headless=headless_mode)
        await connection.page.goto(url, wait_until='domcontentloaded', timeout=settings.timeout)
    ...
    async def _run_script():
        ...
        for i, step in enumerate(script_data['steps']):
            ...
            if 'goto' in step:
                ...
                results.append({'step': i+1, 'action': 'unknown', 'status': 'error', 'error': f'Unknown action: {list(step.keys())[0]}'})
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if attempt < max_attempts:
                    ...
                    output_json({
                        'error': 'Command timed out after all retries',
                        'attempts': max_attempts
                    })
            except Exception as e:
                if attempt < max_attempts:
                    ...
                    output_json({
                        'error': f'Command failed after all retries: {str(e)}',
                        'attempts': max_attempts
                    })
