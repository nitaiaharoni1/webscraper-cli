...
def fetch(
    ...
    navigate_to: Optional[str] = typer.Option(
        None, "--navigate-to", "-n", help="Navigate to this URL first to establish browser context"
    ),
    ...
    async def _fetch():
        connection = await get_connection(session_id, headless, navigate_to)
...
def har(
    ...
    async def _har():
        ...
        # Save HAR
        har_data = {
            "log": {"version": "1.2", "creator": {"name": "agent-service", "version": "1.0"}, "entries": requests_log}
        }

        with open(output_file, "w") as f:
            json.dump(har_data, f, indent=2)

        output_json({"message": f"HAR saved to {output_file}", "requests": len(requests_log)})
...
def mock(
    ...
    async def _mock():
        ...
        async def handle_route(route):
            await route.fulfill(status=status, body=json.dumps(mock_data), headers={"Content-Type": "application/json"})

        await connection.page.route(pattern, handle_route)

        output_json({"message": f"Mocking requests matching: {pattern}", "status": status})

        if url:
            await connection.page.goto(url, wait_until="domcontentloaded")
            output_json({"message": f"Navigated to {url} with mocked endpoint"})
