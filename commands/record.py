...
def stop(
    ...
    async def _stop_recording():
        if not recording_data['is_recording']:
            output_result({'error': 'No recording in progress'})
            return
...
def replay(
    ...
    async def _replay():
        ...
        if not Path(input_file).exists():
            output_result({'error': f'Recording file not found: {input_file}'})
            return
        ...
        for i, action in enumerate(actions):
            ...
            try:
                ...
                output_result({
                    'warning': f'Failed to replay action {i+1}: {str(e)}',
                    'action': action
                })
...
def video_stop(
    ...
    async def _video_stop():
        if not recording_data.get('video_recording'):
            output_result({'error': 'No video recording in progress'})
            return
        ...
        try:
            ...
            output_result({'error': f'Failed to stop video recording: {str(e)}'})

    asyncio.run(_video_stop())
